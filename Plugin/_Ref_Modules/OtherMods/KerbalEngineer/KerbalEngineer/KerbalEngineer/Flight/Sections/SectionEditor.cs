...
public class SectionEditor : MonoBehaviour
{
	...
	{
		GUILayout.BeginHorizontal((GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Height(25f) });
		ParentSection.Name = GUILayout.TextField(ParentSection.Name, textStyle, (GUILayoutOption[])(object)new GUILayoutOption[0]);
		bool flag = !string.IsNullOrEmpty(ParentSection.Abbreviation);
		ParentSection.Abbreviation = GUILayout.TextField(ParentSection.Abbreviation, textStyle, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Width(75f) });
		ParentSection.IsHud = GUILayout.Toggle(ParentSection.IsHud, "HUD", readoutButtonStyle, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Width(50f) });
		if (ParentSection.IsHud)
		{
			ParentSection.IsHudBackground = GUILayout.Toggle(ParentSection.IsHudBackground, "BG", readoutButtonStyle, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Width(50f) });
		}
		if (flag && string.IsNullOrEmpty(ParentSection.Abbreviation))
		{
			DisplayStack.Instance.RequestResize();
		}
		if (GUILayout.Button("DELETE SECTION", readoutButtonStyle, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Width(150f) }))
		{
			ParentSection.IsFloating = false;
			ParentSection.IsEditorVisible = false;
			ParentSection.IsDeleted = true;
			if (SectionLibrary.StockSections.Contains(ParentSection))
			{
				SectionLibrary.StockSections.Remove(ParentSection);
			}
			if (SectionLibrary.CustomSections.Contains(ParentSection))
			{
				SectionLibrary.CustomSections.Remove(ParentSection);
			}
			DisplayStack.Instance.RequestResize();
		}
		GUILayout.EndHorizontal();
	...
	private void DrawPresetSelector()
	{
		...
		((Behaviour)presetList).enabled = GUILayout.Toggle(((Behaviour)presetList).enabled, "▼ PRESETS ▼", categoryTitleButtonStyle, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.Width(150f) });
		if ((int)Event.current.type == 7)
		{
			presetList.SetPosition(GUILayoutUtility.GetLastRect().Translate(position), GUILayoutUtility.GetLastRect());
		}
	}
