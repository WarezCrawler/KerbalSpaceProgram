...
internal class SeriousWarningHandler
{
	...
	private static void UpsertDialog(string message)
	{
		...
		dialog = PopupDialog.SpawnPopupDialog(new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new MultiOptionDialog("B9PartSwitchSeriousWarning", "B9PartSwitch has encountered a serious warning. The game will continue to run but this should be fixed ASAP.\n\n" + string.Join("\n\n", allMessages.ToArray()) + "\n\nPlease see KSP's log for additional details", "B9PartSwitch - Serious Warning", HighLogic.UISkin, new Rect(0.5f, 0.5f, 500f, 60f), new DialogGUIFlexibleSpace(), new DialogGUIHorizontalLayout(new DialogGUIBase[3]
		{
			new DialogGUIFlexibleSpace(),
			new DialogGUIButton("OK", delegate
			{
			}, 140f, 30f, true),
			new DialogGUIFlexibleSpace()
